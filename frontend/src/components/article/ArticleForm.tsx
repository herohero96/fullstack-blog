...
import { getCategories, getTags, createArticle, updateArticle } from '../../lib/api';
...
export default function ArticleForm({ mode, initialData }: ArticleFormProps) {
  ...
  const [selectedTagIds, setSelectedTagIds] = useState<string[]>(
    initialData?.tags?.map((t: Tag) => String(t.id)) || []
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload: Partial<Article> = {
        ...
        content,
        category: categoryId ? ({ id: Number(categoryId) } as Category) : undefined,
        tags: selectedTagIds.map((id) => ({ id: Number(id) } as Tag)),
      ...
      if (mode === 'edit' && initialData) {
        article = await updateArticle(initialData.slug, payload);
      } else {
    ...
    <>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            <label htmlFor="coverImage" className="block text-sm font-medium text-gray-700 mb-1">封面图片链接</label>
            <input
              id="coverImage"
              type="url"
              value={coverImage}
