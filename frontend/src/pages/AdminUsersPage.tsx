              ...
              {filtered.map(user => {
                ...
                return (
                  <tr key={user.id} className="border-b border-gray-100">
                    ...
                    <td className="py-3 text-gray-600">{user.role === 'admin' ? '管理员' : '用户'}</td>
                    <td className="py-3">
                      <span className={`text-xs px-2 py-0.5 rounded font-medium ${badge.className}`}>{badge.text}</span>
                    </td>
                    <td className="py-3 text-gray-500">{user.createdAt ? new Date(user.createdAt).toLocaleDateString() : '-'}</td>
                    <td className="py-3">
